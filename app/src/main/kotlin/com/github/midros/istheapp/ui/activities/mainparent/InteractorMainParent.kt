 ...
 */
class InteractorMainParent<V : InterfaceViewMainParent> @Inject constructor(
    ...
    override fun getStorageReference(child: String): StorageReference =
        firebase().getStorageReference(child)
    ...
    override fun uploadPhotoChild(photo: File) {
        getView()!!.addDisposable(
            firebase().putFile(
                "$CHILD_PHOTO/${photo.toUri().lastPathSegment}",
                photo.toUri()
            ) {
                if (alertDialog != null) alertDialog!!.titleText =
                    "${getContext().getString(R.string.upload_photo_child)} $it%"
            }
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe {
                    if (isNullView()) {
                        alertDialog = getView()!!.showDialog(
                            SweetAlertDialog.PROGRESS_TYPE,
