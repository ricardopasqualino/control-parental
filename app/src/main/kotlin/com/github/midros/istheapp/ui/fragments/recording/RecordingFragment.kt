...
class RecordingFragment : BaseFragment(R.layout.fragment_recording), InterfaceViewRecording{
    ...
    private fun onClick(){
        buttonAddRecord.setOnClickListener {
            if (!recording){
                ...
                showSnackbar(R.string.msj_snackbar_current_record,main)
        }
    }
    ...
    private fun timeConnection() {
        runDelayedOnUiThread(13000) {
            if (viewProgress.isShown)
                failedResult(Throwable(getString(R.string.error_database_connection)))
        }
    }
    ...
    override fun setValueState(dataSnapshot: DataSnapshot) {
        ...
        try {
            if (dataSnapshot.exists())
                toolbar.statePermission = dataSnapshot.value as Boolean
            else
    ...
    override fun setValueTimerRecording(timer: Long) {
        if (timer>0){
            if (!recording)
                toolbar.animateRecord(true)
            toolbar.timer = convertCurrentDuration(timer)
            recording = true
        }else {
            if (recording)
                toolbar.animateRecord(false)
            recording = false
        }
    }
    ...
    private fun itemSelected(key:String?,child:String,file:String){
        if (!key.isNullOrEmpty()){
            ...
            if (dataList.isNotEmpty())
                toolbar.setTitle = "${dataList.size} ${getString(R.string.selected)}"
            else
